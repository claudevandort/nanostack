...
**SQS — feature-complete except for Queue Policy enforcement.** 23 ops (the 17 from v0.3.0 plus the 6 unrouted ops landed in v0.3.2: `ListDeadLetterSourceQueues`, `AddPermission` / `RemovePermission`, and the `MessageMoveTask` trio). FIFO with full per-group ordering + dedup window persistence (v0.3.1). Cold-start safety + MessageRetentionPeriod sweeper landed in v0.3.2. **Remaining gap**: Queue Policy attribute is accepted + mutated by AddPermission/RemovePermission but **not evaluated** at request time — deferred to v0.3.3.
...
| **v0.3.3 — Queue Policy enforcement** | Wire `policy_eval.zig` into a new `src/services/sqs/authz.zig` hook; SQS action map (`sqs:SendMessage` etc.); threading Principal through the SQS service context. Bucket-owner-implicit-allow default. | Last SQS robustness gap. Mirrors the S3 authz pattern but on a smaller surface (no ACLs, no PAB). (Patch.) |
