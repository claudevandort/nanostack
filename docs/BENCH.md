...
Builds nanostack in `ReleaseFast` with strip, then runs `bench/driver.py` (a small Python program that drives signed AWS SDK requests at the binary and measures cold start, idle RSS, request latency, and throughput). Exits 0 if every gated metric is within budget, non-zero otherwise.

Requires `boto3` on the active Python interpreter — install with `pip install -r bench/requirements.txt` (or set `PYTHON=/path/to/venv/bin/python` before invoking).
...
| Warm p99 PutObject (1 KB) | ≤ 10 ms |
| Warm p99 GetObject (≤ 1 MB) | ≤ 5 ms |
| Concurrent PutObject 1 KB throughput | ≥ 150 req/s |
| Multipart upload p99 (5 × 5 MiB, concurrent) | ≤ 500 ms |

Two budgets (`put_object_p99_ms`, `put_object_throughput_rps`) were widened on 2026-05-14 when the bench harness switched from `aws-sdk-go-v2` to `boto3`. The server itself is unchanged — boto3's per-request SigV4 path is heavier than the Go SDK's, and Python's GIL serialises signing across the 32 driver threads. New budgets reflect what the boto3-driven harness actually measures with ~2× headroom for CI runner variance.
...
- **Latencies / throughput** drive signed requests via the AWS Python SDK (`boto3`) so the numbers include the SDK round-trip + SigV4 verification cost — i.e. what users see.
...
|---|---|---|
| Cold start | ~2.5 ms | 500 ms |
| Idle RSS | ~11.7 MB | 30 MB |
| Binary size | ~1.34 MB | 20 MB |
| Wipe-restart | ~145 ms | 500 ms |
| Put p50 (1 KB) | ~2.85 ms | 3 ms |
| Put p99 (1 KB) | ~4.95 ms | 10 ms |
| Get p99 (1 KB) | ~2.46 ms | 5 ms |
| Put throughput (32 workers) | ~267 req/s | 150 req/s |
| Multipart p99 (5 × 5 MiB) | ~228 ms | 500 ms |
| HeadBucket p99 (info) | ~2.4 ms | — |

(Driver is `bench/driver.py` (boto3); harness recalibration 2026-05-14. Pre-recalibration numbers were measured with `aws-sdk-go-v2`, see git history.)
