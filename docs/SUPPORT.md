...
| 20 | Bucket | `CreateBucket` ignores `<LocationConstraint>` body — AWS rejects mismatched constraint with `IllegalLocationConstraintException` | done | [`src/services/s3/mod.zig`](../src/services/s3/mod.zig), [`src/wire/errors.zig`](../src/wire/errors.zig), [`src/server.zig`](../src/server.zig) | [`test_create_bucket.py::test_create_bucket_mismatched_location_constraint_returns_400`](../tests/conformance/python/test_create_bucket.py) |
| 21 | Restore | `RestoreObject` always returns 202 — AWS returns 200 if already restored | done | [`src/services/s3/restore.zig`](../src/services/s3/restore.zig), [`src/storage/fs.zig`](../src/storage/fs.zig), [`src/storage/mod.zig`](../src/storage/mod.zig) | [`test_restore.py::test_restore_object_first_202_second_200`](../tests/conformance/python/test_restore.py) |
| 22 | Bucket | `ListBuckets` doesn't honour `?prefix`, `?bucket-region`, `?max-buckets`, `?continuation-token` (2023 pagination) | done | [`src/services/s3/mod.zig`](../src/services/s3/mod.zig), [`src/wire/s3_responses.zig`](../src/wire/s3_responses.zig) | [`test_list_buckets.py::test_list_buckets_pagination`](../tests/conformance/python/test_list_buckets.py), [`test_list_buckets_prefix_filters`](../tests/conformance/python/test_list_buckets.py) |
...
| GetItem | supported (Item lookup by primary key; full AttributeValue coverage) | M15-items |
| PutItem | supported (auto-overwrite; ReturnValues NONE/ALL_OLD; ConditionExpression in M15-expressions) | M15-items |
| DeleteItem | supported (idempotent; ReturnValues NONE/ALL_OLD; ConditionExpression in M15-expressions) | M15-items |
